          ...
          if (opts.parseTags) {
            replacements.push([
              ...
              (_value: string, tag: string) => {
                ...
                // Store original tag name (not slugified) in frontmatter
                const originalTag = tag
                const tagSlug = slugTag(tag)
                if (file.data.frontmatter) {
                  const noteTags = file.data.frontmatter.tags ?? []
                  // Store original tag, not slugified
                  file.data.frontmatter.tags = [...new Set([...noteTags, originalTag])]
                }

                return {
                  type: "link",
                  url: base + `/tags/${tagSlug}`,
                  data: {
                    hProperties: {
                      className: ["tag-link"],
                    },
                  ...
                  children: [
                    {
                      type: "text",
                      value: originalTag, // Display original tag name
                    },
                  ],
                }
              },
