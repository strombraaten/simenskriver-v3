...
export async function getSatoriFonts(headerFont: FontSpecification, bodyFont: FontSpecification) {
  ...
  // Fallback to Lora if no fonts were loaded (e.g., Charter isn't on Google Fonts)
  // Also track if we're using fallback so we can update font names in JSX
  let usingFallback = false
  if (fonts.length === 0) {
    console.log(
      styleText(
        "yellow",
        `\nWarning: Failed to load fonts ${headerFontName} and ${bodyFontName}. Falling back to Lora.`,
      ),
    )
    usingFallback = true
    // Use Lora as fallback
    const fallbackHeaderPromises = headerWeights.map(async (weight) => {
      const data = await fetchTtf("Lora", weight)
      if (!data) return null
      return {
        name: headerFontName, // Keep original name so JSX matches
        data,
        weight,
        style: "normal" as const,
      }
    ...
    const fallbackBodyPromises = bodyWeights.map(async (weight) => {
      const data = await fetchTtf("Lora", weight)
      if (!data) return null
      return {
        name: bodyFontName, // Keep original name so JSX matches
        data,
        weight,
        style: "normal" as const,
      }
    ...
    const [fallbackHeaderFonts, fallbackBodyFonts] = await Promise.all([
      Promise.all(fallbackHeaderPromises),
      Promise.all(fallbackBodyPromises),
    ])

    fonts = [
      ...fallbackHeaderFonts.filter((font): font is NonNullable<typeof font> => font !== null),
      ...fallbackBodyFonts.filter((font): font is NonNullable<typeof font> => font !== null),
    ]
  }
...
export const defaultImage: SocialImageOptions["imageStructure"] = ({
  ...
  // Calculate reading time (respect hideReadingTime flag)
